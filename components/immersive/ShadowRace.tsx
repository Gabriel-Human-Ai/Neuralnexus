  ...
  return (
    <div ref={ref} className="shadow-race">
      <svg viewBox={`0 0 ${width} ${height}`} aria-hidden="true">
        ...
        <text x="10" y="11" fill="var(--muted)" fontSize="10">bar: 8/10</text>
        ...
        {points.map((point, index) => <motion.circle key={index} cx={point.x} cy={point.y} r="3" fill={point.score >= 8 ? "var(--aurora-a)" : "var(--muted)"} initial={reduce ? false : { opacity: 0, scale: 0.6 }} animate={inView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.6 }} transition={{ ...MOTION.springSoft, delay: reduce ? 0 : index * 0.06 } as any} />)}
      </svg>
      <div className="cost-race" aria-hidden="true">
        <span>{recommendation.fromModel}</span>
        <div><motion.i style={{ width: `${(primary / max) * 100}%` }} animate={applied ? { width: `${(challenger / max) * 100}%`, backgroundColor: "color-mix(in srgb, var(--aurora-a) 35%, transparent)" } : undefined} transition={{ duration: reduce ? 0 : 0.5 }} /></div>
