...
export async function GET(req: Request) {
  ...
  const profileRows = await db.profileSetting.findMany({ where: { profileId, OR: [{ key: { in: KEYS.filter((key) => !isGlobalSettingKey(key)) } }, { key: { startsWith: "STRICT_FACTS_" } }, { key: { startsWith: "EYE_SYNTH_" } }, { key: { startsWith: "EYE_SEEN_" } }, { key: { startsWith: "WORKSPACE_STEPS_" } }, { key: "INDEX_CONSENT" }] } });
  ...
  profileRows.filter((row) => row.key.startsWith("STRICT_FACTS_") || row.key.startsWith("EYE_SYNTH_") || row.key.startsWith("EYE_SEEN_") || row.key.startsWith("WORKSPACE_STEPS_") || row.key === "INDEX_CONSENT").forEach((row) => { out[row.key] = row.value; });
