...
export async function extractSkillRulesFromOutput(outputId: string) {
  ...
  const proposedCount = await db.skillRule.count({ where: { skillId: output.skillId, status: "proposed" } });
  ...
  await db.modelRun.create({ data: { projectId: output.projectId, provider: meta.provider, model: meta.id, inputTokens: result.inputTokens, outputTokens: result.outputTokens, costUsd: estimateCost(meta.id, result.inputTokens, result.outputTokens), purpose: "genome" } });
  const parsed = parseModelJson<{ rules: string[] }>(result.text);
  if (!parsed?.rules?.length) return 0;
  let created = 0;
  const existing = await db.skillRule.findMany({ where: { skillId: output.skillId } });
  ...
  for (const text of parsed.rules.slice(0, 2).map((rule) => rule.slice(0, 140).trim()).filter(Boolean)) {
    ...
    await db.skillRule.create({ data: { skillId: output.skillId, text, source: "learned", sourceOutputId: output.id, status: "proposed" } });
    created++;
  }
  return created;
}
